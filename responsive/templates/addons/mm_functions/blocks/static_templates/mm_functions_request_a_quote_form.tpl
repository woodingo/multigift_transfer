{** block-description:mm_functions_request_a_quote_form **}
...
{if $product.is_custom_made == "Y" && $product.only_by_request == "Y"}
	{assign var="form_page_id" value=54}
	{assign var="request_quote_form_page" value=30|fn_get_page_data}
{else}
	{assign var="form_page_id" value=30}
{/if}
{assign var="form_container_id" value="mm_functions_request_a_quote_form"}
...
<div id="{$form_container_id}">
	{if $page.page_type == $smarty.const.PAGE_TYPE_FORM}
		...
		{if $form_page_id == 54}
			{if $request_quote_form_page.description}
				{* Add by mashmedia: Replace specific codes in form description with mm_seo_builder *}
				<div class="ty-form-builder__description">
					{__('mm_functions_request_a_quote_custom_made_by_request_description')|fn_mm_seo_manager_get_pattern_with_replaced_tags:$product nofilter}
				</div>
			{/if}
		{else}
		    {if $page.description}
	    ...
	    {foreach from=$page.form.elements key="element_id" item="element"}
		    ...
		    {elseif $element.element_type != $smarty.const.FORM_IP_ADDRESS && $element.element_type != $smarty.const.FORM_REFERER}
		        <div class="ty-control-group">
					{if $element.element_id != 388}{* Current URL *}
		            ...
		            {elseif $element.element_type == $smarty.const.FORM_INPUT}
						{if $element.element_id == 388}{* Current URL *}
							<input id="elm_{$element.element_id}" class="ty-form-builder__input-text ty-input-text" size="50" type="hidden" name="form_values[{$element.element_id}]"  value="{$config.current_url|fn_url}">
						{elseif $element.element_id == 144}
							<input id="elm_{$element.element_id}" class="ty-form-builder__input-text ty-input-text" size="50" type="number" {if !empty($product.min_qty)}min="{$product.min_qty}" {/if}value="{$form_values.$element_id}" name="form_values[{$element.element_id}]" />
						{else}
		            ...
		            {elseif $element.element_type == $smarty.const.FORM_EMAIL || $element.element_type == $smarty.const.FORM_NUMBER || $element.element_type == $smarty.const.FORM_PHONE}
						...
						{if $element.element_id == 296}
							<input id="elm_{$element.element_id}" class="ty-input-text" size="50" {if !empty($product.min_qty)}min="{$product.min_qty}" oninvalid="this.setCustomValidity('{__("mm_functions_request_a_quote_custom_made_by_request_amount_invalid", ['[min_qty]' => $product.min_qty])}')"{/if}type="number" name="form_values[{$element.element_id}]" value="{$form_values.$element_id}" />
						{else}
