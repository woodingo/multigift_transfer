...
<div id="cart_items">
    ...
    <thead>
        <tr>
            <th class="ty-cart-content__title ty-left" colspan="2">{__("product")}</th>
    ...
    {if $cart_products}
        {foreach from=$cart_products item="product" key="key" name="cart_products"}
			{assign var="product_color_option_id" value=$product.product_id|fn_mm_color_codes_get_color_option_id_by_product_id}
			{assign var="product_color_option_variant_id" value=""}
			{assign var="obj_id" value=$product.object_id|default:$key}
			{assign var="cart_item_extra_print_info" value=$cart.products.{$obj_id}.extra.mm_print_codes}
			
			{foreach from=$cart_item_extra_print_info.product_options key=option_id item=option_variant_id}
				{if $option_id == $product_color_option_id}
					{assign var="product_color_option_variant_id" value=$option_variant_id}
				{/if}
			{/foreach}
            
            {hook name="checkout:items_list"}
            	{* Add by mashmedia: Link to product with cart id (to fill in product calculation form) *}
				{assign var="mm_print_codes_product_link_with_cart_id" value="{"products.view?product_id=`$product.product_id`"|fn_url}?cid={$obj_id}"}
                {if !$cart.products.$key.extra.parent}
                    <tr>
                            ...
                            {if $runtime.mode == "cart" || $show_images}
                                <div class="ty-cart-content__image cm-reload-{$obj_id}" id="product_image_update_{$obj_id}">
                                    {hook name="checkout:product_icon"}
                                        <a href="{$mm_print_codes_product_link_with_cart_id}">
                                        {assign var="combination_image_pair" value=""}
                                        {assign var="combination_image_params" value=['product_id'=>$product.product_id, 'color_variant_id'=>$product_color_option_variant_id]}
                                        {assign var="combination_image_pair" value=$combination_image_params|fn_mm_functions_get_combination_image_by_color_variant_id}
                                        {include file="common/image.tpl" obj_id=$key images=$combination_image_pair image_width=$settings.Thumbnails.product_cart_thumbnail_width image_height=$settings.Thumbnails.product_cart_thumbnail_height}
                                        </a>
                                    {/hook}
                        ...
                        <td class="ty-cart-content__product-elem ty-cart-content__description ty-table-disable-convertation" style="width: 50%;">
                            {strip}
                                <a href="{$mm_print_codes_product_link_with_cart_id}" class="ty-cart-content__product-title">
                            ...
                            {if $cart.products.$key.extra}
                                <div class="cm-reload-{$obj_id} ty-cart-content__options" id="options_update_{$obj_id}">
									{include file="addons/mm_functions/views/mm_functions/components/order_item_information.tpl" cart_item_extra=$cart.products.$key.extra}
									<div class="ty-control-group product-list-field clearfix"><a href="{$mm_print_codes_product_link_with_cart_id}">{__("mm_print_codes_change_options")}</a></div>
                                </div>
                            {/if}
                                        ...
                                        <div class="ty-group-block hidden" id="discount_{$key}" title="{$price_info_title}">
                                            <table class="ty-cart-content__more-info ty-table">
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="ty-center">{$cart_item_extra_print_info.quantity}</td>
                            ...
                            <div id="options_{$key}" class="ty-product-options ty-group-block hidden">
                                <div class="ty-caret-info-wrapper">
                                    <span class="ty-caret-info ty-light"><span class="caret-outer"></span> <span class="caret-inner"></span></span>
                                </div>
                         ...
                         <td class="ty-cart-content__product-elem ty-cart-content__qty {if $product.is_edp == "Y" || $product.exclude_from_calculate} quantity-disabled{/if}">
                            ...
                            <div class="quantity cm-reload-{$obj_id}{if $settings.Appearance.quantity_changer == "Y"} changer{/if}" id="quantity_update_{$obj_id}">
                            	{$cart_item_extra_print_info.quantity}
                            	{* Edit by mashmedia: no quantity input field, just display *}
    							{*
                        ...
                        <td class="ty-cart-content__product-elem ty-cart-content__price cm-reload-{$obj_id}" id="price_display_update_{$obj_id}">
                        	{* Add by mashmedia: *}
					    	{*<span id="product_price_`$key`" class="sub-price">
					    		{$currencies.$primary_currency.symbol nofilter}{$product.display_price|number_format:4:$currencies.$primary_currency.decimals_separator:$currencies.$primary_currency.thousands_separator}
					    	</span> *}
                        	
	                        {* Edit by mashmedia: Don't display price with price.tpl due to chance of more than one decimal *}
                        ...
                        <!--price_subtotal_update_{$obj_id}-->
    						{assign var="rounding_advantage" value=$cart_item_extra_print_info.rounding_advantage}
							{if $rounding_advantage}
								<br />
								{include file="common/price.tpl" value=$rounding_advantage}&nbsp;<a class="cm-tooltip" title="{__("mm_print_codes_rounding_advantage_tooltip")}"><i class="fa fa-info-circle"></i></a>
							{/if}
                        </td>
                    </tr>
                {/if}
